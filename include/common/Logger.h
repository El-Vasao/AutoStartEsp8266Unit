 ...
 * — `release`: только SSE, без поднятия UART для диагностики.
 * — `logSerialOnly`: только UART в debug-сборке; в release вызовы игнорируются (без SSE, без форматирования в проде).
 * Ограничение по частоте на стороне `Logger::log` снято: при шторме логов нагрузку несёт AsyncEventSource
 * и хост; при необходимости см. очередь `SSE_SOFT_QUEUE_MAX`, `SSE_MAX_QUEUED_MESSAGES`.
 *
...
public:
     ...
     * см. правило в заголовке класса. Доставка: SSE.
     *
