 ...
 * Хранит текущую ошибку и время её возникновения, дублирует в RTC-памяти.
 *
 * Зачем RTC:
 * - если устройство перезагрузилось (WDT/OOM/питание), последняя “важная” ошибка остаётся доступной после старта,
 *   и её можно показать в UI/логах.
 *
 * Ограничение:
 * - RTC-память маленькая и не предназначена для частых/больших записей; здесь хранится только один рекорд.
 */
class ErrorManager {
public:
    ErrorManager();
...
private:
    ...
    static constexpr uint16_t RTC_ADDR = 64;  ///< адрес в RTC-памяти (байт)
};
